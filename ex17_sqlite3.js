...
db.serialize(()=>{
    ...
    VALUES ('Kotlin', 'New Android Programming', 2500, 'available')`

    db.run(sql_create_table)
    // .run(sql_insert_table)
    .each("select * from stock", (err, row)=>{
        if (err) throw err
        console.log(`id: ${row.id}, title: ${row.title}, description: ${row.description}, price: ${row.price}, status: ${row.status}`)
    })
